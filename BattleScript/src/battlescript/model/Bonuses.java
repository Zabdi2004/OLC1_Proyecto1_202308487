...
public final class Bonuses {
    ...
    public Bonuses(List mageCombo, int mageComboPoints, List warriorCombo, int warriorComboPoints, int lowHealthVictory) {
        if (mageComboPoints < 0 || warriorComboPoints < 0 || lowHealthVictory < 0) throw new IllegalArgumentException("Bonificaciones inválidas");
        this.mageCombo = new ArrayList<Action>(mageCombo); this.mageComboPoints = mageComboPoints;
        this.warriorCombo = new ArrayList<Action>(warriorCombo); this.warriorComboPoints = warriorComboPoints;
