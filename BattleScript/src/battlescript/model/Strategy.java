...
public final class Strategy {
    ...
    public Strategy(String name, ClassType classType, Action initialAction, List rules) {
        if (name == null || name.trim().isEmpty()) throw new IllegalArgumentException("El nombre de la estrategia es obligatorio");
        if (initialAction == null || initialAction.getOwner() != classType) throw new IllegalArgumentException("La acción inicial no corresponde a la clase");
        this.name = name;
    ...
    public Action selectAction(Context context) {
        if (context.getRoundNumber() == 0) return initialAction;
        for (Rule rule : rules) {
